...
export default async function ReTr({ n }: ReTrProps) {
  ...
  if (!data || data.length === 0) {
    return (
      <Card
        ...
        className="p-4 shadow-md rounded-xl bg-white m-4 max-w-md mx-auto"
      >
  ...
  return (
    <div className="w-full flex justify-center items-center px-4">
      <Card
        title="💳 Add Money Transactions"
        className="p-6 shadow-md rounded-2xl bg-white max-w-xl w-full"
      >
        <div className="space-y-4">
          {data.slice(0, limit).map((d: Transaction, index: number) => {
            ...
            return (
              <div
                key={index}
                className="flex flex-col md:flex-row justify-between items-start md:items-center p-4 rounded-lg bg-gray-50 border border-gray-200 shadow-sm hover:shadow-md transition-all duration-300"
              >
                {/* Left side */}
                <div className="mb-3 md:mb-0">
                  ...
                  <div className="text-sm text-gray-600">{d.provider}</div>
